...
namespace Microsoft.WindowsAPICodePack.Shell
{
	public class StockIcons
	{
		...
		public StockIcons()
		{
			...
			foreach (StockIconIdentifier item in values)
			{
				stockIconCache.Add(item, null);
			}
		}
		...
		private StockIcon GetStockIcon(StockIconIdentifier stockIconIdentifier)
		{
			...
			StockIcon stockIcon = new StockIcon(stockIconIdentifier, defaultSize, isLinkOverlay, isSelected);
			try
			{
				stockIconCache[stockIconIdentifier] = stockIcon;
			}
			catch
			{
				stockIcon.Dispose();
		...
		{
			StockIconIdentifier[] array = new StockIconIdentifier[stockIconCache.Count];
			stockIconCache.Keys.CopyTo(array, 0);
			StockIconIdentifier[] array2 = array;
			foreach (StockIconIdentifier stockIconIdentifier in array2)
			{
				if (stockIconCache[stockIconIdentifier] == null)
				{
					GetStockIcon(stockIconIdentifier);
				}
			}
